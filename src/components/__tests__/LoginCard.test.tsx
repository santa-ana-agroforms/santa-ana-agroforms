...
describe('LoginCard', () => {
  ...
  test('envía credenciales cuando se llena el formulario (sin fake timers)', async () => {
    ...
    const setTimeoutSpy = jest.spyOn(global, 'setTimeout').mockImplementation(((cb: any, ms?: number, ...args: any[]) => {
      if (typeof ms === 'number' && ms >= 1200) {
        if (typeof cb === 'function') cb(...args);
        else { eval(cb as string); }
        return 0 as any;
      }
    ...
    await waitFor(() =>
      expect(onFinish).toHaveBeenCalledWith({ username: 'mario', password: 'secreta' })
    );
